...
def test_numpy_array_basic_algebra() -> None:
    ...
    # en: Addition
    # ja: 足し算
    assert (np.array([1, 2, 3]) + np.array([1, 2, 3]) == np.array([2, 4, 6])).all()

    # en: Subtraction
    # ja: 引き算
    assert (np.array([4, 5, 6]) - np.array([1, 2, 3]) == np.array([3, 3, 3])).all()

    # en: Multiplication
    # ja: 各成分の掛け算
    assert (np.array([1, 2, 3]) * np.array([2, 3, 4]) == np.array([2, 6, 12])).all()

    # en: Division
    # ja: 各成分の割り算
    assert (np.array([4, 5, 6]) / np.array([1, 2, 3]) == np.array([4, 2.5, 2])).all()
    ...
    # ja: 外積
    assert (
        np.cross(np.array([1, 2, 3]), np.array([4, 5, 6])) == np.array([-3, 6, -3])
    ).all()

    # en: Element-wise power: element-wise power
    # ja: 各成分ごとのべき乗
    assert (np.array([1, 2, 3]) ** np.array([2, 3, 4]) == np.array([1, 8, 81])).all()
...
def test_numpy_types() -> None:
    ...
    # mixed
    x = np.array([1, 2, 3, 4, 5.0])  # en: mixed int and float is converted to float
    assert x.dtype == np.float64
    assert x.size == 5, "size is 5"
    assert x.itemsize == 8, "float64 is 8 bytes"
    assert x.size * x.itemsize == 40, "array byte size is 40"
...
def test_numpy_convenient_methods() -> None:
    ...
    assert x5.std() == 1.707825127659933
    ...
    # ja: 指定した値で埋められた配列を作成する
    x10 = np.array([1, 2, 3])
    x_full_like = np.full_like(x10, 3.14, dtype=np.float64)
